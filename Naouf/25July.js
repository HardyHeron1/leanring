const obj = {
  ...
  obj.product.forEach((p)=>{
    if (obj.product[highestprice].price<p.price){
        highestprice=p;
    }
  })
  console.log(obj.product[highestprice]);

  let highestsalary = 0;
  obj.employee.forEach((p)=>{
    if (obj.employee[highestsalary].salary<p.salary){
        highestsalary=p;
    }
  })
  console.log(obj.employee[highestsalary]);